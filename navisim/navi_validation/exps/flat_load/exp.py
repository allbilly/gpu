...
class FlatLoadExp(Experiment):

    def __init__(self):
        super(FlatLoadExp, self).__init__()
    ...
    def desc(self):
        return "The flat_load microbenchmark tests the performance when the GPU "\
            "runs work-items with only one flat_load instrution followed by a waitcnt.  The x-axis is " \
            "the number of work-groups to launch while the line-series " \
            "represents the number of wavefronts per work-group."
