...
from benchmark import AtaxBenchmark
from benchmark import PRBenchmark
from benchmark import KMeansBenchmark
from benchmark import BicgBenchmark
from benchmark import BSBenchmark
from benchmark import FTBenchmark
from benchmark import FWBenchmark
from benchmark import MTBenchmark
from benchmark import ReluBenchmark
from data import create_output_folder
...
def main():
    ...
    run(benchmarks, args, output_folder)
...
def compile_rest():
    rest = ['atax','bicg','bitonicsort','fastwalshtransform','floydwarshall','matrixtranspose','relu']
    for b in rest:
        path = '../'+str(b)+'/native'
        p = subprocess.Popen('make', shell=True, cwd=path)
        p.wait()
        if p.returncode != 0:
            print("fail to compile "+str(b))
            exit(-1)
...
def setup_benchmarks(benchmarks, args):
    ...
    benchmarks.append(AtaxBenchmark(args))
    benchmarks.append(PRBenchmark(args))
    benchmarks.append(KMeansBenchmark(args))
    benchmarks.append(BicgBenchmark(args))
    benchmarks.append(BSBenchmark(args))
    benchmarks.append(FTBenchmark(args))
    benchmarks.append(FWBenchmark(args))
    benchmarks.append(MTBenchmark(args))
    benchmarks.append(ReluBenchmark(args))
    
def run(benchmarks, args, output_folder):
    """ Run all benchmarks """
