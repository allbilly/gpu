...
func (cu *ComputeUnit) initWfRegs(wf *Wavefront) {
	...
	for i := wf.FirstWiFlatID; i < wf.FirstWiFlatID+32; i++ {
		...
		wf.WriteReg(rdnainsts.VReg(0), 1, laneID, rdnainsts.Uint32ToBytes(uint32(x)))

		if co.EnableVgprWorkItemID() > 0 {
			wf.WriteReg(rdnainsts.VReg(1), 1, laneID, rdnainsts.Uint32ToBytes(uint32(y)))
		}

		if co.EnableVgprWorkItemID() > 1 {
			wf.WriteReg(rdnainsts.VReg(2), 1, laneID, rdnainsts.Uint32ToBytes(uint32(z)))
		}
	}
}
...
func (cu *ComputeUnit) logInst(wf *Wavefront, inst *rdnainsts.Inst) {
	ctx := akita.HookCtx{
		Domain: cu,
		Now:    0,
		Item:   wf,
